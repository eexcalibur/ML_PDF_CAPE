...
def cape_pdf(t,p,q):
    ...
    file_path = "/global/cscratch1/sd/zhangtao/E3SM/20220120.ne4pg2_oQU240.F2010/case_scripts/SourceMods/src.eam/"
    ...
    print("time in python I/O: ", io_time_end - io_time_start)
...
cdef public double* calc_cape(double t[72][12], double p[72][12], 
        double q[72][12]):
    #cape_py = np.zeros(block, dtype=np.float64)
    cdef double [:] cape
    cape = np.zeros(block, dtype=np.float64)

    t_np = np.zeros((lev,block))
    p_np = np.zeros((lev,block))
    q_np = np.zeros((lev,block))
    ...
    for j in range(block):
        cape[j] = cape_pdf(t_np[:,j],p_np[:,j],q_np[:,j])

    end_time = time.time()
    print("time in python: ", end_time - start_time)
    return &cape[0]
